...
export default function WeatherScreen() {
  ...
  async function fetchWeather() {
    ...
    setHasError(false);
    try {
      const res = await api.get('/weather?latitude=14.2117&longitude=121.1653&forecast_days=14');
  ...
  const visual = useMemo(() => getWeatherVisualByCode(data?.current?.weather_code), [data?.current?.weather_code]);
